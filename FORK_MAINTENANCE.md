...
| 生产分支 | `custom/gallery` |
| 上游 | `upstream/main` = `ab99d56e9` (`v0.2.1-1-gab99d56e9`, VERSION `0.2.1`) |
| 定制源码提交 | `ab09c660b1a5` |
| 当前仓库 HEAD | 以 `git rev-parse --short=12 HEAD` 为准 |
| 生产镜像 | `sub2api-custom:20260906-upstream-v021-eceb20b2` |
| Compose 项目 | `peter-sub2api` |
| 本机入口 | `http://127.0.0.1:18080` |
| 公网入口 | `https://api.peterai.cc.cd` |
| 数据 volume | `peter-sub2api_sub2api_data` |
| 最近发布备份 | `/home/aihub/Peter_ws/sub2api-backups/20260906_212001` |
...
```bash
sg docker -c 'docker compose -f deploy/docker-compose.yml --env-file deploy/.env up -d --no-deps --force-recreate sub2api'
sg docker -c 'docker compose -f deploy/docker-compose.yml --env-file deploy/.env ps'
deploy/verify-production.sh
```

`--no-deps` 必须保留；否则依赖服务配置与现有容器有差异时，Compose 可能连带重建 Postgres 或 Redis。
...
- `2026-09-06`：通过 GitHub REST API 复核 `upstream/main` 最新提交为 `ab99d56e9626e6cd731592dae8553c9758a0efa2`（`v0.2.1-1-gab99d56e9`，VERSION `0.2.1`），以 merge commit `ab09c660b1a5` 同步到 `custom/gallery`，合并前备份分支为 `custom/gallery-backup-20260906-before-v0.2.1`。冲突处理保留图片必须有真实输出才计费/成功及无图 failover 语义，同时吸收上游安全 URL 到 `b64_json` 回填、上游请求头记录和 `ReasoningEffort` 计费参数；GPT-5.5、PeterAI 模型发现、Gallery、托管媒体、多级代理、易支付、问候、i18n 与 Canvas 定制均已复核。发布前本地门禁：Go 1.27.0 unit/无标签/integration 全量通过，golangci-lint v2.13.0 为 0 issues，govulncheck v1.7.0 无可达漏洞；前端 ESLint/typecheck、259 文件/1880 测试、定制 i18n 3 文件/15 测试和 production build 通过；Ent/Wire 生成、PeterAI 静态脚本、Docker 快速测试、Compose 安全/环境/资源/Caddy 及 pnpm audit 例外检查通过。首次推送提交 `eceb20b2c2f4` 对应 `CI` run `34035200855` 和 `Security Scan` run `34035200853` 均为 `completed/success`，其中 macOS shell job 完成了本机 Linux 无法执行的 Apple Container 功能验证。发布前备份 `/home/aihub/Peter_ws/sub2api-backups/20260906_212001` 的 SHA-256、1222 项 PostgreSQL TOC 和 48 项应用数据归档校验通过；生产镜像为 `sub2api-custom:20260906-upstream-v021-eceb20b2`，内嵌 VERSION `0.2.1` 和 commit `eceb20b2c2f4`。仅使用 `--no-deps --force-recreate sub2api` 替换应用容器，Postgres、Redis 的容器 ID 与重启次数保持不变。完整生产验收、本机与公网认证请求、Canvas 健康/菜单/iframe 均通过；公网报告版本 `0.2.1`，四项 v0.2.1 数据库迁移已应用，图片选项接口两端均返回 HTTP 200 和 8 个可用 Key，可发现并定价 `gpt-image-1`、`gpt-image-1.5`、`gpt-image-2` 的 `1K`、`2K`、`4K` 层级。新容器日志中该接口 500、`account repository is not configured`、panic、迁移失败和启动错误均为 0。
- `2026-09-03`：通过 GitHub REST API 核对 `upstream/main` 最新提交为 `5097b31457e6dc9f49e5f5c9c72b925ce79543b3`（`v0.2.0-1-g5097b3145`，VERSION `0.2.0`），以 merge commit `ade0df8d7d09` 同步到 `custom/gallery`，合并前备份分支为 `custom/gallery-backup-20260903-before-v0.2.0`。保留 Gallery、托管媒体、图片计费与 failover、GPT-5.5、代理层级、易支付、问候、i18n、PeterAI 多模型生图及 Canvas 定制；修复 PeterAI `/api/v1/user/image-generation/options` 因 `AccountRepository` 未注入导致的 500，模型发现同时识别映射键/值和分组 `models_list_config`，只返回可调度 OpenAI 账号的公开别名，并为空映射/透传账号提供有限图片默认模型；同时恢复 CN adaptive 账号的 OpenAI 协议密钥读取并适配上游新版托管媒体调度签名。生产数据库只读核对确认生图分组可发现 `gpt-image-2`，VIP 生图分组可发现 `gpt-image-1`、`gpt-image-1.5`、`gpt-image-2`。发布前本地门禁：Go 1.27.0 unit/无标签/integration 全量通过、golangci-lint v2.13 为 0 issues、govulncheck 无可达漏洞；前端 ESLint/typecheck、256 文件/1848 测试、定制 i18n 3 文件/15 测试和 production build 通过；Wire 生成、PeterAI 静态脚本、Docker 快速测试、Compose 安全/环境/资源/Caddy、audit 例外及源码不变量检查通过。首次推送提交 `68e4e24aa41d` 对应 `CI` run `33672293505` 和 `Security Scan` run `33672294217` 均为 `completed/success`，其中 macOS shell job 验证了 Apple Container 脚本。发布前备份 `/home/aihub/Peter_ws/sub2api-backups/20260903_032727` 的 SHA-256 与 1192 项 PostgreSQL TOC 校验通过；生产镜像为 `sub2api-custom:20260903-upstream-v020-68e4e24a`。完整生产验收、本机与公网认证请求、Canvas 健康/菜单/iframe 均通过；图片选项接口返回 8 个可用 Key，模型与价格均包含 `gpt-image-1`、`gpt-image-1.5`、`gpt-image-2`，新容器日志中该接口 500、依赖注入错误、panic 和迁移失败均为 0。发布时旧手册命令因未带 `--no-deps` 意外连带重建 Postgres 容器；命名数据卷保持挂载，users/accounts/settings 数据量核验正常，Postgres 随即恢复 healthy，手册发布和回滚命令已补上 `--no-deps` 防止再次发生。
