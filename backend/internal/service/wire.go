...
func ProvideAPIKeyService(
	...
	// PeterAI's options endpoint is served by APIKeyService itself. Wire these
	// dependencies at service construction so a future handler refactor cannot
	// silently leave the endpoint returning "account repository is not configured".
	svc.SetAccountRepository(accountRepo)
	svc.SetImagePricingDependencies(billingService, nil)
	return svc
}
