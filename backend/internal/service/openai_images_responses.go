...
func isOpenAIImagesRetryableRequestRejected(statusCode int, upstreamMsg string, upstreamBody []byte) bool {
	...
	if code == "" {
		code = strings.ToLower(strings.TrimSpace(gjson.GetBytes(upstreamBody, "code").String()))
	}
	if code != "request_rejected" {
		return false
	}
	errType := strings.ToLower(strings.TrimSpace(gjson.GetBytes(upstreamBody, "error.type").String()))
	message := strings.ToLower(strings.TrimSpace(upstreamMsg + " " + extractUpstreamErrorMessage(upstreamBody)))
	if strings.Contains(errType, "content") || strings.Contains(errType, "policy") || strings.Contains(message, "content policy") || strings.Contains(message, "safety") || strings.Contains(message, "policy") {
		return false
	}
	return true
}
...
func (s *OpenAIGatewayService) handleOpenAIImagesOAuthNonStreamingResponse(
	...
	if err != nil {
		if c != nil && c.Request != nil && shouldClassifyOpenAIUpstreamStreamReadError(err, c.Request.Context()) {
	...
	for {
		...
		case ev, ok := <-events:
			...
			if ev.err != nil {
				...
				if c != nil && c.Request != nil && shouldClassifyOpenAIUpstreamStreamReadError(ev.err, c.Request.Context()) {
...
func (s *OpenAIGatewayService) handleOpenAIImagesOAuthResponseError(
	...
	retryable := IsOpenAIImagesRetryableUpstreamError(upstreamErr) ||
		isOpenAIImagesRetryableUpstreamErrorWithBody(upstreamErr, responseBody)
	kind := "http_error"
	if retryable {
		kind = "failover"
		if responseWritten {
