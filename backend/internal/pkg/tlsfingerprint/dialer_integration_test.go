...
func skipExternalNetworkTest(t *testing.T) {
	t.Helper()
	if testing.Short() {
		t.Skip("skipping integration test in short mode")
	}
	if os.Getenv("TLSFINGERPRINT_NETWORK_TESTS") != "1" {
		t.Skip("skipping external TLS fingerprint test; set TLSFINGERPRINT_NETWORK_TESTS=1 to run")
	}
}
