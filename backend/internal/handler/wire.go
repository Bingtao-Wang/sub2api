...
// ProvideAPIKeyHandler completes the channel-aware pricing dependency used by
// the PeterAI image-generation options endpoint. This late wiring avoids the
// APIKeyService -> pricing resolver -> channel service -> APIKeyService
// cache-invalidator cycle; the account repository and base billing service are
// already installed by service.ProvideAPIKeyService.
func ProvideAPIKeyHandler(
	...
	apiKeyService.SetImagePricingDependencies(billingService, pricingResolver)
	return NewAPIKeyHandler(apiKeyService)
}
